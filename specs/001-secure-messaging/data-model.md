...
ALL entity primary keys and foreign keys use `decimal(28,8)` generated by `Toledo.SharedKernel.Helpers.DecimalTools.GetNewId()`. No Guids are used for primary keys. EF Core column type: `.HasColumnType("decimal(28,8)")` with `.HasPrecision(28, 8)`.
...
|-------|------|-------------|
| Id | decimal(28,8) | PK, generated via DecimalTools.GetNewId() |
...
|-------|------|-------------|
| Id | decimal(28,8) | PK, generated via DecimalTools.GetNewId() |
| UserId | decimal(28,8) | FK → User, required, indexed |
...
|-------|------|-------------|
| Id | decimal(28,8) | PK, generated via DecimalTools.GetNewId() |
| DeviceId | decimal(28,8) | FK → Device, required, indexed |
...
| Field | Type | Constraints |
|-------|------|-------------|
| ConversationId | decimal(28,8) | FK → Conversation, PK (composite) |
| UserId | decimal(28,8) | FK → User, PK (composite) |
...
|-------|------|-------------|
| Id | decimal(28,8) | PK, generated via DecimalTools.GetNewId() |
| ConversationId | decimal(28,8) | FK → Conversation, required, indexed |
| SenderDeviceId | decimal(28,8) | FK → Device, required |
| RecipientDeviceId | decimal(28,8) | FK → Device, required, indexed |
...
These entities are stored exclusively in the user's browser. The server never sees this data. Client-side IDs use `decimal` for consistency with server-side IDs.

### LocalIdentity

| Field | Type | Notes |
|-------|------|-------|
| DeviceId | decimal | Matches server Device.Id |
...
| RemoteDeviceId | decimal | The other party's device |
...
| Field | Type | Notes |
|-------|------|-------|
| Id | decimal | Local message ID, generated via DecimalTools.GetNewId() |
| ConversationId | decimal | Matches server Conversation.Id |
...
```
SERVER-SIDE (all IDs are decimal(28,8) via DecimalTools.GetNewId()):
...
CLIENT-SIDE (per device, in IndexedDB — IDs are decimal for consistency):
