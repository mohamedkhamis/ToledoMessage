# Tasks: ToledoVault SaaS Enhancement Plan (v2.0)
...
- [X] T001 Verify existing project builds successfully in src/ToledoVault.Client/ and src/ToledoVault/
...
- [X] T003 [P] Document any existing rate limiting infrastructure in src/ToledoVault/ - RateLimitService.cs found
...
- [X] T004 Add ResponseCompression to Program.cs in src/ToledoVault/Program.cs (FR-011)
- [X] T005 Configure SignalR KeepAliveInterval (30s) and ClientTimeoutInterval (90s) in src/ToledoVault/Program.cs (FR-034)
- [X] T006 [P] Add storage.js setCookie/getCookie helper functions in src/ToledoVault.Client/wwwroot/storage.js (FR-008)
- [X] T007 [P] Add offlineQueue object store to IndexedDB in src/ToledoVault.Client/wwwroot/storage.js (FR-032)
- [X] T008 Update ChatHub to include RateLimitService injection in src/ToledoVault/Hubs/ChatHub.cs (FR-001, FR-002)
...
- [X] T009 [P] [US1] Add rate limiting to ChatHub.SendMessage (60/min) in src/ToledoVault/Hubs/ChatHub.cs
- [X] T010 [P] [US1] Add rate limiting to ChatHub.TypingIndicator (10/min) in src/ToledoVault/Hubs/ChatHub.cs
- [X] T011 [US1] Add rate-limit error handling in SignalRService.cs with countdown timer display in src/ToledoVault.Client/Pages/Chat.razor
- [X] T012 [US1] Add rate-limit warning UI component in MessageInput.razor in src/ToledoVault.Client/Components/MessageInput.razor
- [X] T013 [P] [US1] Add conversation participation check for unread counts in MessagesController.cs in src/ToledoVault/Controllers/MessagesController.cs
- [X] T014 [P] [US1] Add conversation participation check for DeleteForEveryone in ChatHub.cs in src/ToledoVault/Hubs/ChatHub.cs
- [X] T015 [US1] Implement read pointer clamping in MessageRelayService.AdvanceReadPointer in src/ToledoVault/Services/MessageRelayService.cs
- [X] T016 [US1] Sanitize error messages in Login.razor and Register.razor in src/ToledoVault.Client/Pages/Login.razor, src/ToledoVault.Client/Pages/Register.razor
- [X] T017 [US1] Remove any eval() usage in storage.js and replace with setCookie in src/ToledoVault.Client/wwwroot/storage.js
- [X] T018 [US1] Add link preview URL validation (≤2048 chars) in LinkPreviewService.cs in src/ToledoVault/Services/LinkPreviewService.cs
- [X] T019 [US1] Add storage encryption initialization check in LocalStorageService.cs in src/ToledoVault.Client/Services/LocalStorageService.cs
...
- [X] T020 [P] [US2] Implement paginated message loading in Chat.razor (load 50 most recent, "Load earlier messages" button) in src/ToledoVault.Client/Pages/Chat.razor (Blazor Virtualize deferred — pagination approach used instead)
- [X] T021 [US2] Add IndexedDB getMessages with offset/count support in storage.js in src/ToledoVault.Client/wwwroot/storage.js
- [X] T022 [P] [US2] Add display name caching to ChatHub in src/ToledoVault/Hubs/ChatHub.cs
- [X] T023 [P] [US2] Add participant list caching (60s TTL) to ChatHub in src/ToledoVault/Hubs/ChatHub.cs
- [X] T024 [US2] Implement batched delivery acknowledgments in MessagesController.BulkAcknowledgeDelivery in src/ToledoVault/Controllers/MessagesController.cs
- [X] T025 [US2] Add batched MessagesDelivered handler to SignalRService.cs in src/ToledoVault.Client/Services/SignalRService.cs
- [X] T026 [US2] Add debouncing (300ms) to EmojiPicker search in src/ToledoVault.Client/Components/EmojiPicker.razor
- [X] T027 [US2] Add debouncing (200ms) to conversation list filter in ConversationListSidebar.razor in src/ToledoVault.Client/Components/ConversationListSidebar.razor
- [X] T028 [US2] Add random jitter (±30s) to MessageCleanupHostedService in src/ToledoVault/Services/MessageCleanupHostedService.cs
- [X] T029 [US2] Add pre-key cleanup to MessageCleanupHostedService in src/ToledoVault/Services/MessageCleanupHostedService.cs (Deletes all consumed IsUsed=true pre-keys)
- [X] T030 [P] [US2] Optimize user search queries with indexing in src/ToledoVault/Controllers/UsersController.cs
...
- [X] T033 [US3] Add keyboard navigation (arrow keys, Enter) to EmojiPicker in src/ToledoVault.Client/Components/EmojiPicker.razor
- [X] T034 [US3] Make theme cards responsive (reflow on mobile) in Settings.razor (CSS grid already responsive)
- [X] T035 [US3] Make video elements responsive (max-width: 100%) in MessageBubble.razor in src/ToledoVault.Client/Components/MessageBubble.razor
...
- [X] T037 [P] [US3] Complete Arabic localization for all user-visible strings in SharedResource.ar.resx in src/ToledoVault.Shared/SharedResource.ar.resx
...
- [X] T039 [P] [US4] Add offlineQueue JS functions (add, get, update, remove, count) in storage.js in src/ToledoVault.Client/wwwroot/storage.js
- [X] T040 [US4] Add IsConnected property and OnReconnected event to SignalRService in src/ToledoVault.Client/Services/SignalRService.cs
- [X] T041 [US4] Implement offline queue flush on reconnection in SignalRService.cs
- [X] T042 [US4] Modify MessageInput to queue messages when offline in src/ToledoVault.Client/Components/MessageInput.razor
- [X] T043 [US4] Add pending status indicator to queued messages in Chat.razor in src/ToledoVault.Client/Pages/Chat.razor
...
- [X] T047 [US5] Add 5-minute max duration enforcement to voice-recorder.js in src/ToledoVault.Client/wwwroot/voice-recorder.js
- [X] T048 [US5] Add remaining time countdown display to VoiceRecorder.razor in src/ToledoVault.Client/Components/VoiceRecorder.razor
...
- [X] T050 [P] [US6] Add search input UI to Chat.razor in src/ToledoVault.Client/Pages/Chat.razor
...
- [X] T053 [P] [US7] Add admin role check in ChatHub delete operations in src/ToledoVault/Hubs/ChatHub.cs
- [X] T054 [P] [US7] Add promote/demote admin methods in ConversationsController in src/ToledoVault/Controllers/ConversationsController.cs
...
- [X] T056 [P] [US8] Add version byte (0x01) prefix to hybrid signatures in HybridSigner.cs in src/ToledoVault.Crypto/
- [X] T057 [P] [US8] Update signature verifier to handle both v0 and v1 formats in HybridSigner.cs
- [X] T058 [US8] Wire MessageExpiryService to actual message flow in src/ToledoVault.Client/Services/MessageExpiryService.cs
...
- [X] T061 [US9] Verify OnDisconnectedAsync properly calls presence.RemoveConnection in src/ToledoVault/Hubs/ChatHub.cs
