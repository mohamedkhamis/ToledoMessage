...
public class RateLimitServiceTests
{
    ...
    {
        for (int i = 0; i < 5; i++) _service.IsRateLimited("key3", 5, TimeSpan.FromMinutes(1));
    ...
    {
        for (int i = 0; i < 5; i++) _service.IsRateLimited("keyA", 5, TimeSpan.FromMinutes(1));
    ...
    public void IsRateLimited_WindowExpires_ResetsCount()
    {
        ...
        for (int i = 0; i < 5; i++) _service.IsRateLimited("key4", 5, TimeSpan.FromMilliseconds(1));
    ...
    {
        for (int i = 0; i < 3; i++) _service.IsRateLimited("key5", 3, TimeSpan.FromMinutes(1));
