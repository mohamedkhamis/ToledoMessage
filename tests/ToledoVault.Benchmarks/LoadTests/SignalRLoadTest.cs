...
namespace ToledoVault.Benchmarks.LoadTests;
...
///
/// Requirements:
///   - A live ToledoVault server must be reachable at the configured URL.
...
public static class SignalRLoadTest
{
    ...
    public static int Run(string serverUrl, int targetConnections, int durationSeconds)
    {
        ...
        var userPoolSize = Math.Min(targetConnections, 200);
        ...
        var connectScenario = Scenario.Create("signalr_connect", async context =>
            {
                ...
                var connection = new HubConnectionBuilder()
                    .WithUrl(hubUrl, static opts =>
                    {
                        opts.HttpMessageHandlerFactory = static _ =>
                            new HttpClientHandler { ServerCertificateCustomValidationCallback = static (_, _, _, _) => true };
                    })
                    .Build();
        ...
        // -----------------------------------------------------------------------
        var persistentCount = Math.Min(50, tokens.Count);
        ...
        foreach (var c in persistentConns)
            try
            {
                c.DisposeAsync().AsTask().Wait(TimeSpan.FromSeconds(5));
            }
            catch
            {
                /* best-effort */
        ...
        string serverUrl, List<string> tokens, HubConnection[] connections)
    {
        for (var i = 0; i < connections.Length; i++)
        {
            ...
            var conn = new HubConnectionBuilder()
                .WithUrl($"{serverUrl}/hubs/chat?access_token={token}", static opts =>
                {
                    opts.HttpMessageHandlerFactory = static _ =>
                        new HttpClientHandler { ServerCertificateCustomValidationCallback = static (_, _, _, _) => true };
                })
            ...
            {
                 /* leave disconnected; scenario handles it gracefully */
            }
    ...
    private static int EvaluateStats(NodeStats stats)
    {
        ...
        {
            var limitMs = scenario.ScenarioName == "signalr_connect"
                ? ConnectP95LimitMs
                : HubRttP95LimitMs;

            var errorRatePct = scenario.Fail.Request.Percent;
            var p95Ms = scenario.Ok.Latency.Percent95;

            var errorOk = errorRatePct <= MaxErrorRatePct;
            var latencyOk = p95Ms <= limitMs;
