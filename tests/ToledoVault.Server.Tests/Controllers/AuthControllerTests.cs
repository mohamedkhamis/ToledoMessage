...
public class AuthControllerTests
{
    private static (AuthController controller, Data.ApplicationDbContext db) CreateController()
    {
        ...
        var tokenBlacklist = new TokenBlacklistService();
        var controller = new AuthController(db, passwordHasher, config, deletionService, preKeyService, tokenBlacklist, NullLogger<AuthController>.Instance);
    ...
    {
        var (controller, _) = CreateController();
        var result = await controller.Register(new ToledoVault.Shared.DTOs.RegisterRequest("ab", "AB User", "MySecurePass12"));
